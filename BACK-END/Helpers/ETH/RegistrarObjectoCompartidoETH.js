...
const registrarObjectoCompartidoETH = async (
  idPropietario,
  IdObjecto,
  idNuevoUsuario
) => {
  try {
    console.log(
      `ETH registrarObjectoCompartidoETH: -${idPropietario}------------${IdObjecto}-------${idNuevoUsuario}`
    );
    ...
    await contrato.methods
      .set(idPropietario, IdObjecto, idNuevoUsuario)
      .send({ from: signer.address });

    return true;
  } catch (error) {
    console.log(
      "----------------ERROR: registrarObjectoCompartidoETH------------------------- "
    );
    console.log(error);
    return false;
  }
...
export default registrarObjectoCompartidoETH;
